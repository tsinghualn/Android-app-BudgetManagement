        ...
        try
        {
            ...
            while (true) {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    //Used to send the transactions from connector back to the client
                    ...
                    //Used to send the categories from connector back to the client
                    if (message.contains("getcategories;")) {
                        String[] parsed = message.split(";");
                        String username = parsed[1];
                        conn.createUserDB(username);
                        String categories = conn.getCategory();
                        out.println(categories);
                    }
                    //Used to send the goals from connector back to the client
                    if (message.contains("getgoals;")) {
                        String[] parsed = message.split(";");
                        String username = parsed[1];
                        conn.createUserDB(username);
                        String goals = conn.getGoalsList();
                        out.println(goals);
                    }
                }
